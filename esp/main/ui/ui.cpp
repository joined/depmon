...
bool Screen::switchTo(lv_scr_load_anim_t anim_type, uint32_t time, uint32_t delay) {
    if (screen != nullptr) {
...
bool LogsScreen::addLogLine(const string &message) {
    const lock_guard<recursive_mutex> lock(lvgl_mutex);
...
bool LogsScreen::addQRCode(const string &data, const int size) {
    if (panel != nullptr) {
        const lock_guard<recursive_mutex> lock(lvgl_mutex);
...
lv_obj_t *LogsScreen::addNewLine(const string &message) {
    if (panel != nullptr) {
        const lock_guard<recursive_mutex> lock(lvgl_mutex);
...
bool DeparturesScreen::addRandomDepartureItem() {
    return rand() % 2 == 0 ? addItem("M41", "Sonnenallee", "2'") : addItem("U7", "U Rudow", "5'");
};

bool DeparturesScreen::addItem(const string &line_text, const string &direction_text, const string &time_text) {
    if (panel != nullptr) {
